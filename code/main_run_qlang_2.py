...
def get_results(model_name, doc_claim, doc_context, query, src_input_data, lang_to_change, target_tokens, id_to_source, output_file):   
        ...
        if doc_claim:
            for claim in doc_claim:
                ...
                if correct_claim_num is None:
                    ...
                    if lang_to_change != "en":
                        try:
                            ...
                            target_en_title = src_input_data["pos_context_title_en"][int(correct_claim_num)-1]
                            target_en_context = src_input_data["pos_context_en"][int(correct_claim_num)-1]
                ...
                result = {
                    ...
                    "question_src": query,
                    "report_src": src_input_data.get("report_src", ""),
                    "id_to_source": src_input_data.get("id_to_source", ""),
                    "all_claims": src_input_data.get("all_claims", ""),
                    "claims_w_citation": src_input_data.get("claims_w_citation", ""),
                    "claims_retained": src_input_data.get("claims_retained", ""),
...
def get_prefix_decoding(model_name, src_input_file, output_file, lang_to_change):
    existing_ids = get_existing_ids(output_file)
    with open(src_input_file, 'r') as src_fin:
        for src_input_line in src_fin:
            src_input_data = json.loads(src_input_line)
            query = src_input_data.get("question_src", "")
            claims = src_input_data.get("claims_retained", [])
            id_to_source = src_input_data.get("id_to_source", {})

            if src_input_data.get("id") in existing_ids:
                print(f"! Skipping ID: {src_input_data.get('id')} (already processed)")
                continue

            target_tokens = [str(i+1) for i in range(len(src_input_data["pos_context_en"]))]
                
            prompt = make_prompt_main_run(
                context_title=src_input_data["pos_context_title_en"],
                context=src_input_data["pos_context_en"],
            )
...
if __name__ == "__main__":
    ...
    get_prefix_decoding(args.model_id, args.src_input_file, args.output_file, args.lang_to_change)
