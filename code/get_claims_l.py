...
if __name__ == "__main__":
    ...
    with open(args.input_file, 'r', encoding='utf-8') as infile, open(args.output_file, 'a', encoding='utf-8') as outfile:
        for line in infile:
            ...
            doc_context = data.get("pos_context_src", [])
            report = data.get("report_src", "")
            query = data.get("question_src", "")

            # Remove citation from claims (different languages have different claim delimiters)
            if args.lang == "en":
                all_claims, _ = split_by_claims(report)
            elif args.lang == "bn":
                all_claims, _ = split_by_claims_bn(report)
            elif args.lang == "zh":
                all_claims, _ = split_by_claims_zh(report)
            else:
            ...
            for claim in all_claims:
                ...
                if len(context_to_check) == 1:
                    ...
                    print(f"\n! Skipping claim for no/multiple citations: {claim}")
                    continue
