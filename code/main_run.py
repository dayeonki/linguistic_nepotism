...
def get_results(model_name, doc_claim, doc_context, query, input_data, src_input_data, lang_to_change, target_tokens, id_to_source, output_file):   
...
def get_prefix_decoding(model_name, input_file, src_input_file, output_file, lang_to_change):
    ...
    with open(input_file, 'r') as fin, open(src_input_file, 'r') as src_fin:
        for input_line, src_input_line in zip(fin, src_fin):
            ...
            target_tokens = [str(i+1) for i in range(len(input_data["pos_context_en"]))]
...
if __name__ == "__main__":
    ...
    get_prefix_decoding(args.model_id, args.input_file, args.src_input_file, args.output_file, args.lang_to_change)
