...
from utils import make_prompt_main_run_miracl_n1
from transformers import AutoTokenizer, AutoModelForCausalLM, LogitsProcessor


# Troubleshooting
# 1. pip install --upgrade huggingface_hub
# 2. pip install --upgrade safetensors
...
def get_results(model_name, doc_claim, query, input_data, src_input_data, lang_to_change, target_tokens, id_to_source, output_file, distractor_change):   
    with open(output_file, 'a') as fout:
        if doc_claim:
            for claim in doc_claim:
                ...
                # Use source language context and title only for the specific part that needs to be changed
                if lang_to_change != "en":
                    doc_context = make_prompt_main_run_miracl_n1(
                        context_title=input_data["pos_context_title_en"][0],
                        neg_context_title=input_data["neg_context_title_en"][0],
                        context=input_data["pos_context_en"][0],
                        neg_context=src_input_data["neg_context_en"][0],
                    )
                    if distractor_change is True:
                        print("distractor_change is True")
                        target_doc_id = 2
                        change_context = src_input_data["neg_context_src"][0]
                        change_title = src_input_data["neg_context_title_src"][0]

                        target_en_title = input_data["neg_context_title_en"][0]
                        target_en_context = input_data["neg_context_en"][0]
                    else:
                        print("distractor_change is False")
                        target_doc_id = 1
                        change_context = src_input_data["pos_context_src"][0]
                        change_title = src_input_data["pos_context_title_src"][0]
                        
                        target_en_title = input_data["pos_context_title_en"][0]
                        target_en_context = input_data["pos_context_en"][0]
                    ...
                    doc_context = make_prompt_main_run_miracl_n1(
                        context_title=input_data["pos_context_title_en"][0],
                        neg_context_title=src_input_data["neg_context_title_en"][0],
                        context=input_data["pos_context_en"][0],
                        neg_context=src_input_data["neg_context_en"][0],
                    )
                ...
                claim = claim.replace(f" [1].", "").replace(f"[1].", "").replace(f"[1]", "")
                ...
                print(f"✅ Correct Claim to be cited: [1]")
                ...
                result = {
                    ...
                    "question_en": query,
                    "report_en": input_data.get("report_en", ""),
...
def get_prefix_decoding(model_name, input_file, src_input_file, output_file, lang_to_change, distractor_change):
    existing_ids = get_existing_ids(output_file)
    with open(input_file, 'r') as fin, open(src_input_file, 'r') as src_fin:
        for input_line, src_input_line in zip(fin, src_fin):
            ...
            if lang_to_change == "en":
                id_to_source = {"1": "en", "2": "en"}
            else:
                if distractor_change:
                    id_to_source = {"1": "en", "2": "src"}
                else:
                    id_to_source = {"1": "src", "2": "en"}

            if input_data.get("id") in existing_ids:
                print(f"Skipping ID: {input_data.get('id')} (already processed)")
                continue

            # Constrain target tokens to be 1,2
            target_tokens = ["1", "2"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src_input_file", "-s", required=True, help="jsonl file with the source language data")
    ...
    parser.add_argument("--distractor_change", action="store_true", help="change distractor")
    ...
    get_prefix_decoding(args.model_id, args.input_file, args.src_input_file, args.output_file, args.lang_to_change, args.distractor_change)
